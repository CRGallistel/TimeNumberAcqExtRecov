...
%    - valid symbols strings for Matlab plots (e.g, 'k-*' for a black line
%       plot with asterisk markers or 'ro' for a scatter plot with red open
%       circles as data point markers)
...
%
% 'Xcol" and 'Ycol' may be row vectors. If both Xcol and Ycol are specified,
% then they must have the same # of elements, with each element in each
% vector designating a different column in the horizontally concatenated
% data fields passed in. If only a Ycol vector is specified,
% then there will be multiple y-data plots in each panel. In that case, the
% Symbs variable-value pair must also be specified. The value for the
% Symbs variable must be a cell array of valid Matlab plot strings. The cell
% array must contain the same # of cells as the # of data sets to be plotted,
% each cell containing a valid Matlab plot string.
%
...
%
% The default plots are black line plots with asterisk markers. Use the
% Symbs variable-value pair to specify different colors and markers and
% line types (or the absence thereof, as in a scatter plot)
%
% Examples
%
...
%
%   TSapplystat('',{usestat1 usestat2},@TSplot,'Xcol',[1 3],'Ycol',[2 4], ...
%      'Rows',1,'Cols',1,'Symbs',{'k*' 'ro'})
...
%   second with red open circles.
%
%   CRG substantially revised the plotting part of this code 10/6/2019
...
Xlm = [];
Ylm = [];
Symbs = {'-*k'}; % default symbol string in plot command
Ax = [];
FrstRow = 1;
    ...
    for c = NC+1:length(varargin) % stepping through the cells of varargin
        ...
        if ischar(strng)
            ...
            switch  strng % returns the name of a variable
                ...
                case 'Symbs' % scatter plot rather than line plot; the value
...
if size(D,2)>2 && isempty(Ycol)
    sprintf('\nInput error: Data array has more than 2 columns but y-data columns not specified\n')
    OK=false;
elseif numel(Ycol)>size(D,2)
    sprintf('\nInput error: # of Ycols asked for > # data columns\n')
    OK=false;
elseif ~isempty(Ycol) && ~isempty(Xcol) && numel(Xcol)~=numel(Ycol)
    sprintf('\nInput error: # of Ycols specified ~= # of Xcols specified\n')
    OK=false;
elseif  ~isempty(Ycol) && ~isempty(Xcol) && (numel(Xcol)+numel(Ycol))>size(D,2)
     sprintf('\nInput error: More plot columns specified than there are data columns\n')
     OK=false;
elseif numel(Ycol)>1 && length(Symbs)~=numel(Ycol)
    sprintf('\nInput error: When more than 1 y-data column specified,\n# of Symbs strings must = # y-data columns\n')
     OK=false;
end
...
try
    if isempty(Xcol) && ~isempty(Ycol) % only y-data
        D = D(:,Ycol); % if only some of input columns are to be plotted & they
        % are all y-data, this reduces D to only the to-be-plotted columns
    end

    switch size(D,2)    
        case 1 % only 1 column to be plotted
            plot(D(FrstRow:LstRow),Symbs{1})
        case 2 % 2 data columns
            if isempty(Ycol) % default is to plot 2nd col against 1st
                plot(D(FrstRow:LstRow,1),D(FrstRow:LstRow,1),Symbs{1})
            elseif numel(Ycol)<2 % y-x plot
                plot(D(FrstRow:LstRow,2),D(FrstRow:LstRow,1),Symbs{1})
            else % both data columns are Y data
                plot(D(FrstRow:LstRow,1),Symbs{1})
                hold on
                plot(D(FrstRow:LstRow,2),Symbs{2})
            end

        otherwise % Either more than one x-y data plot or more than 2 y-data only
            if isempty(Xcol) % # of symbols specification equals # of data columns
                i = 1;
                for d = Ycol % plot all columns as y-data
                    plot(D(FrstRow:LstRow,d),Symbs{i})
                    hold on
                    i=i+1;
                end
            else % D consists of d pairs of x-y columns, where d==length(Xcol)==length(Symbs)
                for d = 1:numel(Xcol)
                    plot(D(FrstRow:LstRow,Xcol(d)),D(FrstRow:LstRow,Ycol(d)),Symbs{d})
                    hold on
                end
            end
    end
catch ME % error message generated when concatenation fails
    sprintf('\nNew plotting code as of 10/6/2019; probably needs debugging\n')
    disp(getReport(ME)) % display error message
end
    
    
%% OLD CODE    
% if isempty(Xcol) && isempty(Ycol) && (size(D,2)==2) % only 2
%     % columns of data passed in with no Xcol & Ycol specification, default
%     % is to plot second column against first
%     Xcol = 1;
%     Ycol = 2;
% end
% 
% if size(D,2)<2 || (numel(Ycol)==1 && isempty(Xcol)) % Only one data vector,
%     % so plot(Y)
%     if isempty(Ycol)
%         Ycol = 1;
%     end
%     
%     if Scat
%         plot(D(FrstRow:LstRow,Ycol,Symbs{1}))
%     end
% 
% elseif numel(Xcol)<2 % only one plot per panel
%     
